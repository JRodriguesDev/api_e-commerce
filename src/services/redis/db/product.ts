...
import {db_pages_product, db_find_product} from '#prisma_routes/product.js'

export const product_perfil_cache = async (id: string) => {
    const key = `product:${id}`
    const cached = await client.get(key)
    if (cached) return JSON.parse(cached)
    const product = await db_find_product(id)
...
export const product_cache = async (limit: number, skip: number) => {
    ...
    const cached = await client.get(key)
    if (cached) return JSON.parse(cached)
    const products = await db_pages_product(limit, skip)
